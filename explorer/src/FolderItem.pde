...
//  MapLayout algorithm = new PivotBySplitSize();
  ...
  public FolderItem(FolderItem parent, File folder, int level, int order) {
    ...
    if (contents != null) {
      ...
      // return 0 rather than causing an NPE.
  ...
  // overriding super to handle left mouse click
  ...
  void draw() {
    ...
    // modified from book: added ' && this != rootItem' in nonzoom version
    if (contentsVisible) {
      if (mouseInside()) {
        if (parent == zoomItem) {
          taggedItem = this;
  ...
  // overriding super to hide title of parent item
  void drawTitle() {
    if (!contentsVisible) {
      super.drawTitle();
    }
